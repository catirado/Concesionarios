...
namespace Concesionarios.Framework.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        #region Delimiters
        public static string DelimitWith(this string value, char delimiter)
        {
            return value.PadLeft(value.Length + 1, delimiter).PadRight(value.Length + 2, delimiter);
        }

        public static string DelimitWithDoubleQuotes(this string value)
        {
            return value.DelimitWith('\"');
        }
        #endregion
        ...
        public static string ToSpaceCharacterSeparated(this string input)
        {
            var chararray = input.ToCharArray();
            var builder = new StringBuilder();
            var num = chararray.Count();
            int i = 0;

            foreach (var item in chararray)
            {
                builder.Append(item);
                if (i != num) builder.Append(' ');
                i++;
            }
